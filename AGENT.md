...
- **[ハードゲートへのルーティング]**: TDDプロセスやコードの変更・検証を伴う実装ループを回す際は、自己判断でツールを叩かず、今後提供される `tdd_controller.py` 等のシステム的ハードゲート（ステートマシン）を通して実行・遷移すること。
...
5. **[ルールの遵守義務]**: 実装完了時は、必ず上記ガイドラインに基づく厳格なレビューと、`../agent-core/tools/validate_sdd.py` による自動検証を通過させること。
