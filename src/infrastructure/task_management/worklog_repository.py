...
class SQLAlchemyWorklogRepository(WorklogRepository):
    ...
    def save(self, worklog: Worklog) -> None:
        ...
        if not model:
            ...
            if worklog.target_date:
                model = self.session.query(WorklogModel).filter_by(
                    task_id=worklog.task_id, target_date=worklog.target_date
                ).first()
                if not model:
    ...
    def find_by_task_and_date(self, task_id: str, target_date: date) -> List[Worklog]:
        models = self.session.query(WorklogModel).filter(
            WorklogModel.task_id == task_id,
            WorklogModel.target_date == target_date
        ).all()
        return [
            Worklog(
                id=m.id,
