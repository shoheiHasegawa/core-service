...
class SyncWorklogsService:
    def __init__(
        self,
        mobile_vault_repository: MobileVaultRepository,
        task_repository: TaskRepository,
        worklog_repository: WorklogRepository,
        inbox_dir: str,
        archive_dir: str,
    ):
        self.mobile_vault_repository = mobile_vault_repository
    ...
    def sync(self) -> None:
        files = self.mobile_vault_repository.list_markdown_files(self.inbox_dir)
        for filename in files:
            file_path = os.path.join(self.inbox_dir, filename)
            content = self.mobile_vault_repository.read_text(file_path)
            ...
            self.mobile_vault_repository.move_file(file_path, archive_path)
