...
class MobileVaultService:
    ...
    def retrieve_packets(self) -> int:
        files = self.repository.list_markdown_files(self.config.inbox_dir)
        processed_count = 0
    ...
    def place_dashboard(self, content: str, filename: str) -> str:
        self.repository.ensure_directory_exists(self.config.dashboard_dir)
        self.repository.save_file(content=content, directory=self.config.dashboard_dir, filename=filename)
