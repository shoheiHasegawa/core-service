...
def create_task(id: str, category: TaskCategory, estimated_minutes: int, **kwargs) -> Task:
    return Task(id=id, title=f"Task {id}", category=category, estimated_minutes=estimated_minutes, **kwargs)
...
def test_task_10_circadian_dip_handling():
    ...
    schedule_item = {"task": task, "start": datetime(2026, 7, 19, 13, 30), "end": datetime(2026, 7, 19, 14, 30)}
...
def test_task_12_morning_shallow_work_error():
    ...
    schedule_item = {"task": task, "start": datetime(2026, 7, 19, 10, 0), "end": datetime(2026, 7, 19, 11, 0)}
